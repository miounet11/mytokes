#!/bin/bash
# AI History Manager v2.0 - 模块化版本启动脚本
# 使用新的 app.main:app 入口
...
check_dependencies() {
    ...
    if ! python3 -c "from app.main import app" 2>/dev/null; then
        log_error "app 模块加载失败，请检查 app/ 目录"
        exit 1
    fi
...
# 启动服务
start_server() {
    log_step "启动 AI History Manager v${VERSION}..."
...
health_check() {
    ...
    while [ $retry -lt $max_retries ]; do
        # 检查进程是否存在
        if ! pgrep -f "uvicorn $APP_MODULE.*--port $PORT" > /dev/null; then
            log_error "服务进程不存在"
            tail -30 "$LOG_FILE"
            exit 1
        fi
...
show_status() {
    ...
    local main_pid=$(pgrep -f "uvicorn $APP_MODULE.*--port $PORT" | head -1)
    echo "  主进程 PID: $main_pid"

    local worker_count=$(pgrep -f "uvicorn $APP_MODULE" | wc -l)
...
# 显示帮助
show_help() {
    echo -e "${CYAN}AI History Manager v${VERSION} 启动脚本${NC}"
    echo ""
    echo "用法: $0 [命令]"
    echo ""
    echo "命令:"
...
check_status() {
    if pgrep -f "uvicorn $APP_MODULE.*--port $PORT" > /dev/null; then
        log_info "服务运行中 (v${VERSION})"
        show_status
    elif pgrep -f "uvicorn api_server:app.*--port $PORT" > /dev/null; then
        log_warn "运行的是旧版本 (api_server:app)"
        log_info "使用 '$0 restart' 切换到新版本"
    else
        log_warn "服务未运行"
        exit 1
    fi
...
case "${1:-start}" in
    start)
        echo -e "${CYAN}==================== AI History Manager v${VERSION} ====================${NC}"
        force_stop
        check_dependencies
        check_config
        start_server
